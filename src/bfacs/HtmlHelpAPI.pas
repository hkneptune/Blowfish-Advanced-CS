...
function HtmlHelp_ShowContext(nContext : Integer ) : Boolean;
{$ENDIF}
...
function HtmlHelp_ShowContext(nContext : Integer) : Boolean;
begin
  if (0 = nContext) then begin
    Result:=True; 
    Exit;
  end;

  Result:=0 <> HtmlHelp(
    0, PChar(Application.HelpFile), HH_HELP_CONTEXT, nContext);

  if (not Result) then begin
    Result:=0 <> HtmlHelp(
      0, PChar(Application.HelpFile), HH_HELP_CONTEXT, 999);
  end;
end;
