...
bool TestCipherServer
(CTestStdOut* tso)
{
	int nI, nLegacy, lCipherModeFlags;
	int nNumOfCiphers;
	WORD32 lI;
	WORD32 lResult;
	WORD32 lKeySize;
	...
	for (nLegacy = 0, lCipherModeFlags = 0; nLegacy < 2; nLegacy++)
	{
		if (nLegacy)
		{
			tso->Puts("--------NOW TESTING LEGACY CODE...--------");
			lCipherModeFlags |= CIPHER_MODE_FLAG_LEGACY;
		}
	...
	for (nI = 0; nI < nNumOfCiphers; nI++)
	{
		...
		lResult = ::CipherServer_OpenSession(
			CIPHERSERVER_MODE_ENCRYPT | lCipherModeFlags,
			_key,
			lKeySize,
			pCtx,
			pInitData,
		...
		int nNumOfBlocks = (int)::strlen((char*)_inbuf) + 1;
		...
		lResult = ::CipherServer_OpenSession(
			CIPHERSERVER_MODE_DECRYPT | lCipherModeFlags,
			_key,
			lKeySize,
			pCtx,
			pInitData,
