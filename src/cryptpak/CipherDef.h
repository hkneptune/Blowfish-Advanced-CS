...
// work modes (range: 0..65535)
#define CIPHER_MODE_ENCRYPT		0
#define CIPHER_MODE_DECRYPT		1

// the upper 16 bits represent a bitfield, so we define macros to deal with that
#define CIPHER_GETMODE(m)		((m) & 0x0000ffff)
#define CIPHER_GETFLAGS(m)		((m) & 0xffff0000)

// mode flag: the cipher should behave compatible to what was the original behavior,
// even if things like byte ordering are platform-dependent
#define CIPHER_MODE_FLAG_LEGACY		0x00010000
